        ...
        <>
            ...
            <div className='h-full bg-zinc-900 rounded-lg flex flex-col'>
                ...
                <ScrollArea className='flex-1'>
                    {isLoading ? (
                        <Loader className='animate-spin m-auto mt-[30%] size-5 text-emerald-600' />
                    ) : (
                        <>
                        ...
                        {users.map((user) => {
                            ...
                            <div key={user._id} className='cursor-pointer hover:bg-zinc-800/50 p-3 rounded-md transition-colors group'>
                                <div className='flex items-start gap-3'>
                                    ...
                                    <div className={`absolute bottom-0 right-0 size-3 rounded-full border-2 border-zinc-800 ${onlineUsers.has(user.clerkId) ? "bg-green-500" : "bg-zinc-500"}`} aria-hidden='true' />
                                    </div>
                                    <div className='flex-1 min-w-0'>
                                        <div className='flex items-center gap-2'>
                                            <span className='font-medium text-sm text-white capitalize'>{user.fullName}</span>
